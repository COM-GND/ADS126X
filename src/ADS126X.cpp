...
int32_t ADS126X::readADC1(uint8_t pos_pin, uint8_t neg_pin)
{
  ...
  if (REGISTER.INTERFACE.bit.STATUS)
    i++;  // place to hold status byte
  i += 4; // place to hold adc data
  if (REGISTER.INTERFACE.bit.CRC > 0)
    i++; // place to hold checksum/crc byte
...
uint8_t ADS126X::find_crc(uint32_t val, uint8_t byt)
{
  ...
  while ((num & fin) ^ num)
  {                                          // while num is greater than 8 bits
